...
// generates an axios request when request options are passed. requestOptions object must contain url and optional encoding
const generateAxiosRequestByOptions = (requestOptions) => {
  const responseType = requestOptions.encoding === BINARY_ENCODING ?  ARRAY_BUFFER_ENCODING : requestOptions.encoding;
...
(async bindImages => {
  try {
    const firstRequest = generateAxiosRequestByOptions({ url: generateUrl(greeting), encoding: BINARY_ENCODING});
    const secondRequest = generateAxiosRequestByOptions({ url: generateUrl(who), encoding: BINARY_ENCODING});

    const responses = await axios.all([firstRequest, secondRequest]);

    if (responses[0].status == "200" && responses[1].status == "200") {
      console.log("Succesfully received responses from CataaS");
